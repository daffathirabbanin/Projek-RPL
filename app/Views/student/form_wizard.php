...
<header class="bg-white/80 backdrop-blur-md px-8 py-5 border-b border-slate-200/60 flex justify-between items-center sticky top-0 z-10">
  <div>
    <h2 class="text-xl font-black text-slate-800 tracking-tight">FORMULIR PENDAFTARAN</h2>
    <p class="text-[11px] text-slate-400 font-bold uppercase tracking-[0.15em] mt-0.5">Lengkapi data pada setiap tahap</p>
  </div>
...
<div id="notif-error" class="fixed top-6 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-4 bg-white border border-red-200 shadow-2xl rounded-2xl px-6 py-4 min-w-[340px] animate-notif">
  ...
  <div class="flex-1">
    <p class="font-black text-red-600 text-sm">Formulir Belum Lengkap!</p>
    <p class="text-slate-500 text-xs mt-0.5">Pastikan seluruh data wajib (*) sudah diisi dengan benar.</p>
  </div>
...
<?php
$steps = ['Data Pribadi','Data Ayah','Data Ibu','Data Wali','Kontak & Periodik'];
$icons = ['fa-user','fa-male','fa-female','fa-user-friends','fa-phone-alt'];
...
<?php
$p = $data['pribadi'] ?? [];
function inp($name,$label,$val,$type='text',$req=true){
  $reqAttr = $req ? 'required' : '';
  $label .= $req ? ' *' : '';
  echo "<div><label class='block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5'>$label</label><input type='$type' name='$name' value='".htmlspecialchars($val??'')."' class='w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm bg-slate-50/50 hover:bg-white transition-colors' $reqAttr></div>";
}
...
inp('nisn','NISN',$p['nisn']??'');
inp('nik','NIK (16 digit) *',$p['nik']??'');
inp('no_kk','No. Kartu Keluarga',$p['no_kk']??'');
...
<?php inp('rt','RT',$p['rt']??''); inp('rw','RW',$p['rw']??''); inp('dusun','Dusun',$p['dusun']??''); inp('kelurahan','Kelurahan/Desa',$p['kelurahan']??''); ?>
</div>
...
sel('moda_transportasi','Moda Transportasi',$p['moda_transportasi']??'',['Jalan Kaki'=>'Jalan Kaki','Sepeda'=>'Sepeda','Motor'=>'Motor','Angkot'=>'Angkot','Mobil'=>'Mobil']);
?>
</div>
</div></div>
...
inp('nama_ayah','Nama Ayah *',$ay['nama_ayah']??'');
inp('nik_ayah','NIK Ayah',$ay['nik_ayah']??'');
inp('tahun_lahir_ayah','Tahun Lahir',$ay['tahun_lahir_ayah']??'','number');
sel('pendidikan_ayah','Pendidikan',$ay['pendidikan_ayah']??'',$pendidikan);
inp('pekerjaan_ayah','Pekerjaan',$ay['pekerjaan_ayah']??'');
sel('penghasilan_bulanan_ayah','Penghasilan Bulanan',$ay['penghasilan_bulanan_ayah']??'',$penghasilan);
...
<?php
$ib=$data['ibu']??[];
inp('nama_ibu','Nama Ibu *',$ib['nama_ibu']??'');
inp('nik_ibu','NIK Ibu',$ib['nik_ibu']??'');
inp('tahun_lahir_ibu','Tahun Lahir',$ib['tahun_lahir_ibu']??'','number');
sel('pendidikan_ibu','Pendidikan',$ib['pendidikan_ibu']??'',$pendidikan);
inp('pekerjaan_ibu','Pekerjaan',$ib['pekerjaan_ibu']??'');
sel('penghasilan_bulanan_ibu','Penghasilan Bulanan',$ib['penghasilan_bulanan_ibu']??'',$penghasilan);
...
inp('nik_wali','NIK Wali',$wl['nik_wali']??'','text',false);
...
<div class="flex justify-between mt-2">
  ...
  <div class="flex space-x-3">
    <button type="button" onclick="saveAndNext(5)" class="px-6 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl text-sm uppercase hover:bg-slate-200 transition-all border border-slate-200">Lewati <i class="fas fa-forward ml-1"></i></button>
...
inp('lingkar_kepala','Lingkar Kepala (cm)',$pe['lingkar_kepala']??'');
...
<div id="confirm-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 backdrop-blur-sm transition-opacity duration-300 opacity-0">
  <div id="confirm-modal-box" class="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-sm w-full mx-4 transform scale-95 transition-transform duration-300">
    <div class="p-8 text-center">
      <div class="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-5 border-4 border-white shadow-lg">
        <i class="fas fa-paper-plane text-3xl ml-[-2px] mt-[2px]"></i>
      </div>
      <h3 class="text-xl font-black text-slate-800 mb-2 tracking-tight uppercase">Sudah Yakin Datanya Sesuai?</h3>
      <p class="text-sm text-slate-500 font-medium mb-8 leading-relaxed">Mohon periksa kembali ya! Setelah formulir ini dikirim, data tidak bisa diubah lagi karena akan langsung masuk tahap verifikasi panitia.</p>
      
      <div class="flex space-x-3">
        <button type="button" onclick="closeModal()" class="flex-1 py-3.5 bg-slate-100 text-slate-600 font-bold rounded-xl text-sm uppercase tracking-wider hover:bg-slate-200 transition-colors">Cek Lagi</button>
        <button type="button" id="btn-kirim" onclick="submitFinalForm()" class="flex-1 py-3.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold rounded-xl text-sm uppercase tracking-wider hover:from-emerald-700 hover:to-teal-700 transition-all shadow-md shadow-emerald-200/50">
          <span id="btn-kirim-label"><i class="fas fa-paper-plane mr-2"></i>Ya, Kirim Data</span>
          <span id="btn-kirim-loading" class="hidden"><i class="fas fa-circle-notch fa-spin mr-2"></i>Mengirim...</span>
        </button>
      </div>
    </div>
  </div>
